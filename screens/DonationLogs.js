...
export default function DonationLogs({navigation}) {
    ...
    function getDonationData(userType,email,name) {
        setUserType(userType);
        if(userType == "User" || userType == "user"){
            const donationConnect = rdb.ref('donation') ;
            donationConnect.orderByChild('email').equalTo(email)
            .on('value',snapshot => {
                var items = []
            ...
            const donationConnect = rdb.ref('donation') ;
            donationConnect.orderByChild('orphanageName').equalTo(name)
            .on('value',snapshot => {
                var items = []
                snapshot.forEach((child) =>{
                    items.push({
            ...
            const donationConnect = rdb.ref('donation') ;
            donationConnect.once('value').then(snapshot => {
                var items = []
                snapshot.forEach((child) =>{
                    items.push({
                        id: child.key,
    ...
    useEffect(() => {
        ...
        AsyncStorage.getItem('email')
            .then((val) => {
                ...
                getDonationData(user,email,name)
        })
    ...
    function renderAdmin(){
        if(userType == "Admin" || userType == "admin" || userType == "User" || userType == "user"){
            return <View style={{alignContent:'center',alignItems:'center'}}>
                        <TextInput
                        placeholder="Search your Donation here "
    ...
    function moveToEventScreen(DonationData) {
  ...
  return (
    <ScrollView style={globalStyles.primaryContainer}>
        ...
        <Text style= {globalStyles.headingText}>Donation Logs</Text>
        </View>
